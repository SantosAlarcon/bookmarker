...
export async function GET(
	...
	if (!token) {
		return Response.json({ message: "You need to provide an access token to access this resource.", status: 401 });
	}
	...
	if (userId) {
		...
		return Response.json({ message: "ID is required to retrieve bookmark data", status: 400 });
	}
}
